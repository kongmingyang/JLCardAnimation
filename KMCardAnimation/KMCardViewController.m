...
@interface KMCardViewController()<JLDragCardDelegate>
...
@implementation KMCardViewController
...
-(void)refreshAllCards{
    ...
    for (int i=0; i<_allCards.count ;i++) {
        ...
        KMDragCardView *card=self.allCards[i];
...
-(void)loadAllCards{
    ...
    for (int i=0; i<self.allCards.count; i++) {
        KMDragCardView *draggableView=self.allCards[i];
    ...
    for (int i=0; i<_allCards.count ;i++) {
        ...
        [UIView animateKeyframesWithDuration:0.5 delay:0.06*i options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
            ...
            if (i>0&&i<CARD_NUM-1) {
                KMDragCardView *preDraggableView=[_allCards objectAtIndex:i-1];
...
-(void)addCards{
    for (int i = 0; i<CARD_NUM; i++) {
        ...
        KMDragCardView *draggableView = [[KMDragCardView alloc]initWithFrame:CGRectMake([[UIScreen mainScreen]bounds].size.width+CARD_WIDTH, self.view.center.y-CARD_HEIGHT/2, CARD_WIDTH, CARD_HEIGHT)];
...
#pragma mark - 滑动后续操作
-(void)swipCard:(KMDragCardView *)cardView Direction:(BOOL)isRight {
    ...
    for (int i = 0; i<CARD_NUM; i++) {
        KMDragCardView*draggableView=[_allCards objectAtIndex:i];
        draggableView.originalCenter=draggableView.center;
        draggableView.originalTransform=draggableView.transform;
        if (i==0) {
            draggableView.canPan=YES;
...
-(void)moveCards:(CGFloat)distance{
    ...
    if (fabs(distance)<=PAN_DISTANCE) {
        for (int i = 1; i<CARD_NUM-1; i++) {
            KMDragCardView *draggableView=_allCards[i];
            KMDragCardView *preDraggableView=[_allCards objectAtIndex:i-1];
...
-(void)rightButtonClickAction {
    ...
    _flag = YES;
    KMDragCardView *dragView=self.allCards[0];
